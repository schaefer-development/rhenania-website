...
import type { ObjectWithObjectID, Settings as IndexSettings } from '@algolia/client-search';
...
export async function indexAlgolia({ appId, apiKey, indices, ...rest }: Config): Promise<void> {
	const client = algoliasearch(appId, apiKey);
	const config = { ...defaultConfig, ...rest };

	if (config.verbosity > 0) {
		if (config.partialUpdates) console.log(`svelte-algolia: Partial updates enabled`);
		console.log(
			`svelte-algolia: ${indices.length} ${indices.length > 1 ? `indices` : `index`} to update`
		);
	}
...
async function callGetter(getter: IndexConfig[`getData`]): Promise<ObjectWithObjectID[] | void> {
	try {
...
async function overwriteUpdate(
	...
	try {
		...
		if (config.verbosity > 0) console.log(`index '${name}': wrote ${data.length} items`);
	} catch (err) {
		console.error(err);
...
async function partialUpdate(
	...
	const toIndex = data.filter((newObj) => {
		...
		if (matchFields) {
			...
			if (matchFields.some((field) => existingObj[field] !== newObj[field])) return true;
		} else {
...
async function fetchExistingData(index: SearchIndex, attributesToRetrieve: string[]) {
